...
RfidCBusComm::RfidCBusComm() : cbus(CBusInterface::instance()) {}

void RfidCBusComm::initialise() {
    ...
    qDebug() << "Madhur AppManIf::getInstance()->notifyPublished rfidservice";

    QTimer::singleShot(REGISTER_SERV_DELAY,[this](){
        if(!cbus.registerService(RFID_SERV, this, this))
        {
            qDebug() << "RFID_SERV Already regitered";
        }
...
void RfidCBusComm::establishCommunication(void)
{
    qDebug() <<__FUNCTION__ ;
    // Subscribe
...
void RfidCBusComm::heartbeat() {
    qDebug() <<__FUNCTION__ ;
    AppManIf::getInstance()->sendHeartbeatResponse();
}

void RfidCBusComm::handlePowerRestartNotification(StateChange state)
{
    qDebug() <<  __FUNCTION__;
    switch (state)
    {
...
RfidCBusComm::~RfidCBusComm() { cbus.unregisterService(RFID_SERV); }

MessagePtr RfidCBusComm::deserialize(quint16 messageType, QDataStream &stream) {
...
bool RfidCBusComm::messageHandler(quint16 requestId, MessagePtr message) {
...
bool RfidCBusComm::messageHandler(MessagePtr message) {
...
//-------------------------------------------------------
MessagePtr RfidCBusComm::subscriberDeserialize(quint16 messageType,
                                             QDataStream &stream) {
    ...
    case BLSTARTUP_INTIALIZATION: {
        ...
        message->deserialize(stream);
        return MessagePtr(message);
        qDebug() << "Madhur RFID Service subscriberDeserialize - BLSTARTUP_INTIALIZATION";
    }
    default:
        break;
    }
...
//-------------------------------------------------------
bool RfidCBusComm::subscriberMessageHandler(MessagePtr message) {
    ...
    case BLSTARTUP_INTIALIZATION: {
        ...
        return true;
        qDebug() << "Madhur RFID Service subscriberMessageHandler - BLSTARTUP_INTIALIZATION";
...
//-----------------------------------------------------------
void RfidCBusComm::sendReply(quint16 requestId, MessagePtr const &message) {
    cbus.sendReply(requestId, message);
}

bool RfidCBusComm::callService(quint8 serviceName, MessagePtr const &message) {
    return cbus.callService(serviceName, message);
}

void RfidCBusComm::push(MessagePtr const &message) { cbus.push(message); }
