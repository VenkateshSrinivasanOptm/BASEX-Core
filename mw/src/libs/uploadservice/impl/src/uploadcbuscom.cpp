...
UploadCBusComm::UploadCBusComm() : cbus(CBusInterface::instance()) {}

void UploadCBusComm::initialise() {
    ...
    QTimer::singleShot(REGISTER_SERV_DELAY,[this](){
        if(!cbus.registerService(UPLOAD_SERV, this, this))
        {
            qDebug() << "UPLOAD_SERV is already registered.";
        }
    });
...
void UploadCBusComm::establishCommunication(void)
{
    qDebug() <<__FUNCTION__ ;
    // Subscribe
...
void UploadCBusComm::heartbeat() {
    qDebug() <<__FUNCTION__ ;
    AppManIf::getInstance()->sendHeartbeatResponse();
}

void UploadCBusComm::handlePowerRestartNotification(StateChange state)
{
    qDebug() <<  __FUNCTION__;
    switch (state)
    {
...
UploadCBusComm::~UploadCBusComm() { cbus.unregisterService(UPLOAD_SERV); }

MessagePtr UploadCBusComm::deserialize(quint16 messageType, QDataStream &stream) {
...
bool UploadCBusComm::messageHandler(quint16 requestId, MessagePtr message) {
...
bool UploadCBusComm::messageHandler(MessagePtr message) {
...
//-------------------------------------------------------
MessagePtr UploadCBusComm::subscriberDeserialize(quint16 messageType,
                                                 QDataStream &stream) {
...
//-------------------------------------------------------
bool UploadCBusComm::subscriberMessageHandler(MessagePtr message) {
...
//-----------------------------------------------------------
void UploadCBusComm::sendReply(quint16 requestId, MessagePtr const &message) {
    cbus.sendReply(requestId, message);
}

bool UploadCBusComm::callService(quint8 serviceName, MessagePtr const &message) {
    return cbus.callService(serviceName, message);
}

void UploadCBusComm::push(MessagePtr const &message) { cbus.push(message); }
